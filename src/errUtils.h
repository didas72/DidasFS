...
#define ERR_NULL_FREE1(errIfNull, errCode, toFree1, fmt, ...) {\
	if (!errIfNull) { fprintf(stderr, fmt, ##__VA_ARGS__); free(toFree1); return errCode;}}
#define ERR_NULL_FREE2(errIfNull, errCode, toFree1, toFree2, fmt, ...) {\
	if (!errIfNull) { fprintf(stderr, fmt, ##__VA_ARGS__); free(toFree1); free(toFree2); return errCode;}}
#define ERR_NULL_FREE3(errIfNull, errCode, toFree1, toFree2, toFree3, fmt, ...) {\
	if (!errIfNull) { fprintf(stderr, fmt, ##__VA_ARGS__); free(toFree1); free(toFree2); free(toFree3); return errCode;}}
#define ERR_NULL_CLEANUP(errIfNull, errCode, cleanup, fmt, ...) {\
	if (!errIfNull) { fprintf(stderr, fmt, ##__VA_ARGS__); cleanup; return errCode;}}
#define ERR_NULL_CLEANUP_FREE1(errIfNull, errCode, cleanup, toFree1, fmt, ...) {\
	if (!errIfNull) { fprintf(stderr, fmt, ##__VA_ARGS__); cleanup; free(toFree1); return errCode;}}

#define ERR_NZERO(errIfNZero, errCode, fmt, ...) {\
	if (errIfNZero) return errCode; }
#define ERR_NZERO_FREE1(errIfNZero, errCode, toFree1, fmt, ...) {\
	if (errIfNZero) { fprintf(stderr, fmt, ##__VA_ARGS__); free(toFree1); return errCode;}}
#define ERR_NZERO_FREE2(errIfNZero, errCode, toFree1, toFree2, fmt, ...) {\
	if (errIfNZero) { fprintf(stderr, fmt, ##__VA_ARGS__); free(toFree1); free(toFree2); return errCode;}}
#define ERR_NZERO_FREE3(errIfNZero, errCode, toFree1, toFree2, toFree3, fmt, ...) {\
	if (errIfNZero) { fprintf(stderr, fmt, ##__VA_ARGS__); free(toFree1); free(toFree2); free(toFree3); return errCode;}}
#define ERR_NZERO_CLEANUP(errIfNZero, errCode, cleanup, fmt, ...) {\
	if (errIfNZero) { fprintf(stderr, fmt, ##__VA_ARGS__); cleanup; return errCode;}}
#define ERR_NZERO_CLEANUP_FREE1(errIfNZero, errCode, cleanup, toFree1, fmt, ...) {\
	if (errIfNZero) { fprintf(stderr, fmt, ##__VA_ARGS__); cleanup; free(toFree1); return errCode;}}
